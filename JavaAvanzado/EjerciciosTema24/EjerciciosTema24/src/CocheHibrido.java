// Clase CocheHibrido que extiende la funcionalidad de la clase Coche sin modificarla
class CocheHibrido extends Coche {
    public CocheHibrido(String marca, String modelo, int año, double precio) {
        super(marca, modelo, año, precio);
    }

    // Método específico para coche híbrido
    public void estaCargandoBateria(Boolean check) {
        if (check) {
            System.out.println("Esta cargando la bateria...");
        } else {
            System.out.println("La bateria no necesita cargar");
        }
    }
}